...
export default function AdminPembayaranPage() {
  ...
  useEffect(() => {
    ...
    const loadRegistration = async () => {
      ...
      setApprovedBySport((nextExtra.requestItems ?? []).reduce((acc: ApprovedDraft, item: ExtraAthleteAccessItem) => { const baseSlots = Number(item.approvedSlots ?? item.requestedSlots ?? 0); acc[item.sportId] = item.sportId === "voli_indoor" ? String(baseSlots / 12) : String(baseSlots); return acc }, {} as ApprovedDraft))
        ...
        <div className="rounded-2xl border bg-white p-6 shadow-sm space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-xl border bg-gray-50 p-4">
              ...
              <div className="mt-1 text-xs text-gray-500">Cabor diajukan: {extraRequestItems.length > 0 ? extraRequestItems.map((item) => `${item.sportName} (${item.sportId === "voli_indoor" ? item.requestedSlots / 12 + " tim / " + item.requestedSlots + " atlet" : item.requestedSlots + " peserta"})`).join(", ") : registration.extraAthleteAccess?.requestedSportName ?? "-"}</div>
            </div>
            <div className="rounded-xl border bg-gray-50 p-4">
          ...
          <div className="grid grid-cols-1 gap-4 rounded-xl border p-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              ...
              {extraRequestItems.length > 0 ? <div className="mb-2 flex flex-wrap gap-2 text-xs"><span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-slate-700">Total diminta: <b>{extraRequestItems.reduce((total, item) => total + item.requestedSlots, 0)}</b></span><span className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-emerald-700">Total disetujui: <b>{extraRequestItems.reduce((total, item) => total + Math.max(0, item.sportId === "voli_indoor" ? Number(approvedBySport[item.sportId] ?? ((item.approvedSlots ?? item.requestedSlots ?? 0) / 12)) * 12 : Number(approvedBySport[item.sportId] ?? item.approvedSlots ?? item.requestedSlots ?? 0)), 0)}</b></span></div> : null}
              {extraRequestItems.length > 0 ? <div className="mb-3 text-[11px] text-gray-500">Top-up tambahan akan dihitung dari total slot yang disetujui di tiap cabor.</div> : null}
              {extraRequestItems.length > 0 ? <div className="mb-3 inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-800">Estimasi top-up aktif: Rp {(extraRequestItems.reduce((total, item) => total + Math.max(0, item.sportId === "voli_indoor" ? Number(approvedBySport[item.sportId] ?? ((item.approvedSlots ?? item.requestedSlots ?? 0) / 12)) * 12 : Number(approvedBySport[item.sportId] ?? item.approvedSlots ?? item.requestedSlots ?? 0)), 0) * 150000).toLocaleString("id-ID")}</div> : null}
              {extraRequestItems.length > 0 ? <div className="grid gap-3 md:grid-cols-2">{extraRequestItems.map((item) => <div key={item.sportId} className="rounded-xl border bg-gray-50 p-4"><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-bold text-gray-900">{item.sportName}</div><div className="mt-1 text-xs text-gray-500">Diminta: {item.sportId === "voli_indoor" ? item.requestedSlots / 12 + " tim / " + item.requestedSlots + " atlet" : item.requestedSlots + " peserta"}</div></div><div className="rounded-full border border-emerald-200 bg-white px-3 py-1 text-[11px] font-bold text-emerald-700">Disetujui {item.sportId === "voli_indoor" ? Math.max(0, Number(approvedBySport[item.sportId] ?? ((item.approvedSlots ?? item.requestedSlots ?? 0) / 12))) + " tim / " + Math.max(0, Number(approvedBySport[item.sportId] ?? ((item.approvedSlots ?? item.requestedSlots ?? 0) / 12))) * 12 + " atlet" : Math.max(0, Number(approvedBySport[item.sportId] ?? item.approvedSlots ?? item.requestedSlots ?? 0)) + " peserta"}</div></div><div className="mt-3 text-[11px] font-semibold text-gray-600">{item.sportId === "voli_indoor" ? "Jumlah tim disetujui" : "Jumlah peserta disetujui"}</div><input type="number" min={0} value={approvedBySport[item.sportId] ?? (item.sportId === "voli_indoor" ? String((item.approvedSlots ?? item.requestedSlots ?? 0) / 12) : String(item.approvedSlots ?? item.requestedSlots ?? 0))} onChange={(e) => setApprovedBySport((prev) => ({ ...prev, [item.sportId]: e.target.value }))} className="mt-2 w-full rounded-lg border px-3 py-2" /></div>)}</div> : <input type="number" min={0} value={approvedSlots} onChange={(e) => setApprovedSlots(e.target.value)} className="w-full rounded-lg border px-3 py-2" />}
            </div>
            <div>
            ...
            <button
              onClick={() => {
                ...
                const normalizedRequestItems = requestItems.map((item: ExtraAthleteAccessItem) => ({
                  ...item,
                  approvedSlots: extraStatus === "OPEN" ? Math.max(0, item.sportId === "voli_indoor" ? Number(approvedBySport[item.sportId] ?? ((item.requestedSlots ?? 0) / 12)) * 12 : Number(approvedBySport[item.sportId] ?? item.requestedSlots ?? 0)) : 0,
                }))
                ...
                const updated = withExtraFlow({
                  ...
                  topUpPayment: {
                    ...
                    additionalFee: normalizedApprovedSlots * 150000,
