        ...
        <div className="rounded-2xl border bg-white p-6 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 rounded-xl border p-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              ...
              {extraRequestItems.length > 0 ? <div className="mb-3 inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-800">Estimasi top-up aktif: Rp {(extraRequestItems.reduce((total, item) => total + Math.max(0, item.sportId === "voli_indoor" ? Number(approvedBySport[item.sportId] ?? ((item.approvedSlots ?? item.requestedSlots ?? 0) / 12)) * 12 : Number(approvedBySport[item.sportId] ?? item.approvedSlots ?? item.requestedSlots ?? 0)), 0) * 100000).toLocaleString("id-ID")}</div> : null}
