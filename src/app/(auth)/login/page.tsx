      ...
      <div className="grid w-full max-w-5xl gap-5 lg:grid-cols-[1.1fr_1fr]">
        ...
        <section className="surface-card w-full p-6 md:p-7">
          ...
          <p className="mt-1 text-sm text-gray-600">
            {mode === "LOGIN" ? "Masuk untuk melanjutkan pendaftaran Muhammadiyah Games 2026." : "Reset password menggunakan kode dari panitia/admin."}
          </p>

          <div className="mt-5 grid grid-cols-2 gap-2 rounded-xl border bg-gray-50 p-1">
            <button type="button" onClick={() => setMode("LOGIN")} className={mode === "LOGIN" ? "rounded-lg bg-white px-3 py-2 text-sm font-extrabold text-emerald-800 shadow" : "rounded-lg px-3 py-2 text-sm font-bold text-gray-600"}>Login</button>
            <button type="button" onClick={() => setMode("FORGOT")} className={mode === "FORGOT" ? "rounded-lg bg-white px-3 py-2 text-sm font-extrabold text-emerald-800 shadow" : "rounded-lg px-3 py-2 text-sm font-bold text-gray-600"}>Lupa Password</button>
          </div>
          ...
          {mode === "LOGIN" ? (
            ...
            <div className="mt-5 space-y-3">
              <p className="text-sm text-gray-600">Minta kode reset ke panitia/admin, lalu isi data berikut.</p>
